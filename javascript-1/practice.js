...
let arrayReverser = (arr) => {
  ...
  for (let i = arr.length - 1; i >= 0; i--) {
    // remember to i needs to be equal to 0 as well as greater than.
    reversed.push(arr[i]);
  }
  return reversed;
};
...
const filtered = myNumbers.filter((element) => {
  if (element > 100) {
    return true; //I am returning all the elements in the array that are true
  }
});
...
// Code Here
const total = myNumbers.reduce((acc, element) => acc + element); //Remember to  put in the accumulator as a parameter.
...
const enemies = people.filter((element) => {
  if (element.friend === false) {
    // Stepping down using element to access friend value.
    return true;
  }
});
//////////////////PROBLEM 20////////////////////
...
// Code Here
let totallyAwesome = people.reduce((acc, current) => {
  return acc += current.awesomeLevel; // don't forget the return and also make accumulator  a number
}, 0);
